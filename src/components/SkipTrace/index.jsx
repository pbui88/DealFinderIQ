...
function StatusBadge({ status }) {
  const cfg = {
    saved:      { label: 'Saved',      cls: 'bg-slate-100 text-ink-muted border border-line' },
    submitted:  { label: 'Submitted',  cls: 'bg-tagYellow-bg text-tagYellow-text border border-tagYellow-text/20' },
    processing: { label: 'Processing', cls: 'bg-tagBlue-bg text-tagBlue-text border border-tagBlue-text/20' },
    completed:  { label: 'Completed',  cls: 'bg-tagGreen-bg text-tagGreen-text border border-tagGreen-text/20' },
    failed:     { label: 'Failed',     cls: 'bg-tagRed-bg text-tagRed-text border border-tagRed-text/20' },
  }[status] || { label: status, cls: 'bg-slate-100 text-ink-muted border border-line' }
  return (
    ...
    <div className="min-h-full bg-paper">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        ...
        <div className="flex items-center gap-3 mb-8">
          <button
            onClick={openSidebar}
            className="p-2 rounded-full text-ink-muted hover:text-ink hover:bg-paper-bone transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98] lg:hidden shrink-0"
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2.5 mb-1">
              ...
              <h1 className="font-display text-xl sm:text-2xl font-bold text-ink tracking-tight">Skip Trace</h1>
            </div>
            <p className="text-sm text-ink-muted">Find property owner contact info — phones, emails &amp; more</p>
          </div>
          ...
          <div className="shrink-0 flex items-center gap-2">
            ...
            {!isAdmin && usage && (
              <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs border ${
                skipTraceBalance <= 0
                  ? 'bg-tagRed-bg border-tagRed-text/20 text-tagRed-text'
            ...
            {records.some(r => r.status === 'submitted' || r.status === 'processing') && (
              <button
                ...
                disabled={checking}
                className="flex items-center gap-2 px-3 py-2 rounded-full bg-tagGreen-bg border border-tagGreen-text/20 text-sm text-tagGreen-text hover:bg-emerald-100 transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98] disabled:opacity-50"
              >
                {checking ? (
            ...
            <button
              ...
              disabled={uploading}
              className="flex items-center gap-2 px-3 py-2 rounded-full border border-line bg-white hover:bg-paper-bone text-sm text-ink transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98] disabled:opacity-50"
            >
              {uploading ? (
                <span className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />
              ) : (
        ...
        <AnimatePresence initial={false}>
          {submitResult?.message && (
            <motion.div
              ...
              className="flex items-center gap-3 bg-tagGreen-bg border border-tagGreen-text/20 rounded-2xl px-4 py-3.5 mb-5"
            >
              <CheckCircleIcon weight="fill" className="w-4 h-4 text-tagGreen-text shrink-0" />
              <p className="text-sm text-tagGreen-text font-medium flex-1">{submitResult.message}</p>
              <button onClick={() => setSubmitResult(null)} className="text-tagGreen-text/70 hover:text-tagGreen-text p-1 rounded-full transition-all duration-300 active:scale-[0.98]"><XIcon weight="light" className="w-4 h-4" /></button>
            </motion.div>
          )}
          {submitResult && !submitResult.message && (
            <motion.div
              ...
              exit={reduce ? { opacity: 0 } : { opacity: 0, y: -8 }}
              transition={{ duration: 0.3, ease: EASE }}
              className="flex items-start gap-3 bg-tagGreen-bg border border-tagGreen-text/20 rounded-2xl px-4 py-3.5 mb-5"
            >
              <CheckCircleIcon weight="fill" className="w-4 h-4 text-tagGreen-text shrink-0 mt-0.5" />
              <div className="flex-1">
                <p className="text-sm text-tagGreen-text font-medium">
                  <span className="font-bold">{submitResult.recordCount} record{submitResult.recordCount !== 1 ? 's' : ''}</span> submitted for skip trace.
                </p>
                <p className="text-xs text-tagGreen-text/70 mt-0.5">Results will appear here once processing is complete. This typically takes a few minutes.</p>
                {submitResult.skippedIncomplete > 0 && (
                  <p className="text-xs text-tagYellow-text mt-1">
              ...
              <button onClick={() => setSubmitResult(null)} className="text-tagGreen-text/70 hover:text-tagGreen-text p-1 rounded-full shrink-0 transition-all duration-300 active:scale-[0.98]"><XIcon weight="light" className="w-4 h-4" /></button>
            </motion.div>
          )}
          {dncSubmitResult?.message && (
            <motion.div
              ...
              exit={reduce ? { opacity: 0 } : { opacity: 0, y: -8 }}
              transition={{ duration: 0.3, ease: EASE }}
              className="flex items-center gap-3 bg-violet-50 border border-violet-200 rounded-2xl px-4 py-3.5 mb-5"
            >
              {dncPolling ? (
                <span className="w-4 h-4 border-2 border-violet-600 border-t-transparent rounded-full animate-spin shrink-0" />
              ) : (
                <CheckCircleIcon weight="fill" className="w-4 h-4 text-violet-600 shrink-0" />
              )}
              ...
              {!dncPolling && (
                <button onClick={() => setDncSubmitResult(null)} className="text-violet-500 hover:text-violet-700 p-1 rounded-full transition-all duration-300 active:scale-[0.98]"><XIcon weight="light" className="w-4 h-4" /></button>
              )}
            </motion.div>
          )}
          {(submitError || uploadError || dncSubmitError) && (
            <motion.div
              ...
              exit={reduce ? { opacity: 0 } : { opacity: 0, y: -8 }}
              transition={{ duration: 0.3, ease: EASE }}
              className="flex items-center gap-3 bg-tagRed-bg border border-tagRed-text/20 rounded-2xl px-4 py-3.5 mb-5"
            >
              <WarningCircleIcon weight="light" className="w-4 h-4 text-tagRed-text shrink-0" />
              <p className="text-sm text-tagRed-text font-medium flex-1">
                {submitError || uploadError || dncSubmitError}
                {(submitError || dncSubmitError || '').startsWith('Insufficient') && (
                  <> — <button onClick={() => navigate('/credits')} className="underline hover:text-tagRed-text/80">Deposit funds →</button></>
                )}
              </p>
              <button onClick={() => { setSubmitError(null); setUploadError(null); setDncSubmitError(null) }} className="text-tagRed-text/70 hover:text-tagRed-text p-1 rounded-full transition-all duration-300 active:scale-[0.98]"><XIcon weight="light" className="w-4 h-4" /></button>
            </motion.div>
          )}
          {checkResult && !checkResult.error && (
            ...
            >
              <CheckCircleIcon weight="fill" className="w-4 h-4 text-tagGreen-text shrink-0" />
              <p className="text-sm text-tagGreen-text font-medium flex-1">
              ...
              <button onClick={() => setCheckResult(null)} className="text-tagGreen-text/70 hover:text-tagGreen-text p-1 rounded-full shrink-0 transition-all duration-300 active:scale-[0.98]"><XIcon weight="light" className="w-4 h-4" /></button>
            </motion.div>
          )}
          {checkResult?.error && (
            <motion.div
            ...
            >
              <WarningCircleIcon weight="light" className="w-4 h-4 text-tagRed-text shrink-0" />
              <p className="text-sm text-tagRed-text font-medium flex-1">{checkResult.error}</p>
              <button onClick={() => setCheckResult(null)} className="text-tagRed-text/70 hover:text-tagRed-text p-1 rounded-full shrink-0 transition-all duration-300 active:scale-[0.98]"><XIcon weight="light" className="w-4 h-4" /></button>
            </motion.div>
          )}
        </AnimatePresence>

        {/* CSV format hint */}
        <div className="flex items-start gap-2.5 bg-paper-bone border border-line rounded-2xl px-4 py-3 mb-6 text-xs text-ink-muted">
          <InfoIcon weight="light" className="w-4 h-4 text-ink-muted shrink-0 mt-0.5" />
          <span>
            CSV columns: <span className="text-ink font-mono">address</span>, <span className="text-ink font-mono">city</span>, <span className="text-ink font-mono">state</span>, <span className="text-ink font-mono">zip</span>.
            Only <span className="text-ink font-mono">address</span> is required. The filename becomes the list name.
        ...
        {(checkedSaved.length > 0 || checkedCompleted.length > 0) && (
          <div className="sticky top-4 z-10 flex flex-wrap items-center gap-3 bg-white/80 backdrop-blur-2xl border border-line rounded-2xl px-5 py-3.5 mb-6 shadow-[0_4px_16px_rgba(15,23,42,0.06)]">
            ...
            {checkedCompleted.length > 0 && (
              <div className="flex items-center gap-2 flex-1 min-w-0 flex-wrap">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-ink-muted">Ready to export</p>
                </div>
                ...
                {selectedHasDnc && (
                  <button
                    ...
                    className="shrink-0 flex items-center gap-2 px-4 py-2 rounded-full bg-tagGreen-bg hover:bg-emerald-100 border border-tagGreen-text/25 text-tagGreen-text text-sm font-semibold transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98]"
                  >
            ...
            <button
              ...
              className="shrink-0 p-1.5 rounded-full text-ink-muted hover:text-ink hover:bg-paper-bone transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98]"
              title="Deselect all"
            >
        ...
        {showConfirm && (() => {
          ...
          return (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 backdrop-blur-sm px-4">
              <div className="bg-slate-100 border border-line p-1.5 rounded-[1.75rem] max-w-sm w-full">
                <div className="bg-white rounded-[calc(1.75rem-0.375rem)] p-6">
                  <h3 className="font-display text-base font-bold text-ink mb-3">Confirm Skip Trace</h3>
                  <div className="bg-paper-bone border border-line rounded-xl p-4 mb-5 space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-ink-muted">Records</span>
                      <span className="text-ink font-semibold font-mono">{checkedSaved.length}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-ink-muted">Rate</span>
                      <span className="text-ink font-semibold font-mono">$0.08 / record</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-ink-muted">Est. Cost</span>
                      <span className="text-brand-600 font-bold font-mono">${traceCost.toFixed(2)}</span>
                    </div>
                    {!isAdmin && (
                      <>
                        <div className="flex justify-between text-sm pt-1 border-t border-line">
                          <span className="text-ink-muted">Your Balance</span>
                          <span className={`font-semibold font-mono ${canAfford ? 'text-ink' : 'text-tagRed-text'}`}>
                            ${skipTraceBalance.toFixed(2)}
                          </span>
                        </div>
                        {canAfford ? (
                          <div className="flex justify-between text-sm">
                            <span className="text-ink-muted">After this job</span>
                            <span className="text-ink font-semibold font-mono">${(skipTraceBalance - traceCost).toFixed(2)}</span>
                          </div>
                        ) : (
                          <div className="pt-1">
                            <p className="text-xs text-tagRed-text">
                              Insufficient funds — you need ${(traceCost - skipTraceBalance).toFixed(2)} more.{' '}
                              <button onClick={() => { setShowConfirm(false); navigate('/credits') }} className="underline hover:text-tagRed-text/80">Deposit funds →</button>
                            </p>
                          </div>
                        )}
                      </>
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => setShowConfirm(false)} className="flex-1 py-2.5 rounded-full border border-line bg-white hover:bg-paper-bone text-ink text-sm font-medium transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98]">Cancel</button>
        ...
        {showDncConfirm && (() => {
          ...
          return (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 backdrop-blur-sm px-4">
              <div className="bg-slate-100 border border-line p-1.5 rounded-[1.75rem] max-w-sm w-full">
                <div className="bg-white rounded-[calc(1.75rem-0.375rem)] p-6">
                  <h3 className="font-display text-base font-bold text-ink mb-1">Confirm DNC Scrub</h3>
                  <p className="text-xs text-ink-muted mb-2">Checks phone numbers against these databases:</p>
                  ...
                  <div className="bg-paper-bone border border-line rounded-xl p-4 mb-5 space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-ink-muted">Records</span>
                      <span className="text-ink font-semibold font-mono">{dncCandidates.length}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-ink-muted">Phones to check</span>
                      <span className="text-ink font-semibold font-mono">{totalPhonesForDnc}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-ink-muted">Rate</span>
                      <span className="text-ink font-semibold font-mono">$0.02 / phone</span>
                    </div>
                    <div className="flex justify-between text-sm pt-1 border-t border-line">
                      <span className="text-ink-muted">Est. Cost</span>
                      <span className="text-violet-700 font-bold font-mono">${dncCost.toFixed(2)} <span className="text-ink-faint font-normal text-[10px]">max</span></span>
                    </div>
                    {!isAdmin && (
                      <>
                        <div className="flex justify-between text-sm">
                        ...
                        {!canAfford && (
                          <div className="pt-1">
                            <p className="text-xs text-tagRed-text">
                              ...
                              <button onClick={() => { setShowDncConfirm(false); navigate('/credits') }} className="underline hover:text-tagRed-text/80">Deposit funds →</button>
                            </p>
                          </div>
                        )}
                      </>
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => setShowDncConfirm(false)} className="flex-1 py-2.5 rounded-full border border-line bg-white hover:bg-paper-bone text-ink text-sm font-medium transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98]">Cancel</button>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <p className="text-tagRed-text text-sm mb-3">{error}</p>
          ...
          <div className="space-y-4">
            {groups.map(group => {
              ...
              return (
                <div key={group.key} className="bg-white/80 backdrop-blur-2xl border border-line rounded-2xl overflow-hidden shadow-[0_4px_16px_rgba(15,23,42,0.06)]">
                  {/* Group header */}
                  <div className="px-4 py-3 flex items-center gap-3">
                    ...
                    >
                      <CaretRightIcon
                        weight="bold"
                        className={`w-3.5 h-3.5 text-ink-muted shrink-0 transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] ${isExpanded ? 'rotate-90' : ''}`}
                      />
                      <span className="text-sm font-semibold text-ink truncate">{group.name}</span>
                      <div className="flex items-center gap-1.5 shrink-0 flex-wrap">
                        <span className="text-[10px] text-ink-muted font-mono">{group.records.length} record{group.records.length !== 1 ? 's' : ''}</span>
                        {group.savedCount > 0 && (
                          <span className="text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded-full bg-slate-100 text-ink-muted border border-line font-semibold">{group.savedCount} ready</span>
                        )}
                        {group.submittedCount > 0 && (
                          <span className="text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded-full bg-tagYellow-bg text-tagYellow-text border border-tagYellow-text/20 font-semibold">{group.submittedCount} processing</span>
                        )}
                        {group.completedCount > 0 && (
                          <span className="text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded-full bg-tagGreen-bg text-tagGreen-text border border-tagGreen-text/20 font-semibold">{group.completedCount} done</span>
                        )}
                      </div>
                    </button>
                    ...
                    <button
                      ...
                      className="shrink-0 p-1.5 rounded-full text-ink-muted hover:text-tagRed-text hover:bg-paper-bone transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98] disabled:opacity-40"
            ...
            <div className="text-center">
              <button onClick={load} className="flex items-center gap-1.5 mx-auto text-xs text-ink-muted hover:text-ink transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98]">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className={`w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98] shrink-0
            ${open ? 'bg-violet-600 text-white' : 'bg-slate-100 text-ink-muted hover:bg-slate-200 hover:text-ink'}`}
        >?</button>
      </div>
      {open && (
        <p className="text-[11px] text-ink-muted leading-relaxed mt-1.5 pl-3.5 pr-1">{info}</p>
      )}
    </div>
  )
}
...
function RecordRow({ record, checked, onCheck, onDelete, deletingId }) {
  ...
  return (
    <div className={`flex items-start gap-3 px-4 py-3 transition-colors duration-300 ${checked ? 'bg-brand-50' : 'hover:bg-paper-bone'}`}>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm text-ink truncate font-medium">
          {record.address
            ...
            : <span className="text-ink-muted italic">No address</span>}
        </p>
        <div className="flex items-center gap-1 mt-0.5">
          <span className="text-[10px] text-ink-faint font-mono">{new Date(record.created_at).toLocaleDateString()}</span>
        </div>
        {record.status === 'completed' && (
          record.result
            ? <ContactResult result={record.result} record={record} />
            : <p className="text-xs text-ink-faint mt-1.5 italic">No contact data found</p>
        )}
      </div>
      ...
      <div className="shrink-0 flex items-center gap-2">
        ...
        {isSaved && (
          <button
            ...
            disabled={isDeleting}
            className="p-1 rounded-full text-ink-muted hover:text-tagRed-text hover:bg-paper-bone transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98] disabled:opacity-40"
            title="Remove"
          >
            {isDeleting ? (
              <span className="w-3.5 h-3.5 border border-current border-t-transparent rounded-full animate-spin block" />
...
function PhoneTag({ type }) {
  if (type === 'primary')  return <span className="text-[10px] font-semibold uppercase tracking-wide text-tagGreen-text border border-tagGreen-text/30 bg-tagGreen-bg rounded-full px-1.5 py-0.5">Primary</span>
  if (type === 'landline') return <span className="text-[10px] font-semibold uppercase tracking-wide text-ink-muted border border-line bg-paper-bone rounded-full px-1.5 py-0.5">Landline</span>
  return                          <span className="text-[10px] font-semibold uppercase tracking-wide text-brand-600 border border-brand-200 bg-brand-50 rounded-full px-1.5 py-0.5">Mobile</span>
}

function DncFlag({ value }) {
  if (value === true)      return <span className="inline-block text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-tagRed-bg text-tagRed-text border border-tagRed-text/20">Y</span>
  if (value === false)     return <span className="inline-block text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-slate-100 text-ink-muted border border-line">N</span>
  return                          <span className="inline-block text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-paper-bone text-ink-faint border border-line">–</span>
}
...
function ContactResult({ result, record }) {
  ...
  if (!result.full_name && !phones.length && !emails.length) {
    return <p className="text-xs text-ink-faint mt-1.5 italic">No contact data found</p>
  }
  ...
  return (
    <div className="mt-3 space-y-2.5">
      ...
      <div className="bg-paper-bone border border-line rounded-xl p-3">
        <div className="flex items-center justify-between mb-2.5">
          <span className="text-[10px] font-bold text-ink-muted uppercase tracking-widest">Skip Trace Result</span>
          <button
            onClick={downloadSkipTrace}
            className="flex items-center gap-1 text-[10px] text-ink-muted hover:text-ink transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98]"
        ...
        {result.full_name && (
          <div className="flex items-center gap-1.5 mb-2.5">
            <UserIcon weight="light" className="w-3 h-3 text-ink-muted shrink-0" />
            <span className="text-xs font-semibold text-ink">{result.full_name}</span>
          </div>
        )}

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="col-span-2 lg:col-span-1">
            <p className="text-[10px] font-bold text-ink-muted uppercase tracking-widest mb-1.5">Phones</p>
            {phones.length === 0 ? (
              ...
              <div className="space-y-1.5">
                {phones.map((ph, i) => (
                  <div key={i} className="flex items-center gap-1.5">
                    <span className="text-xs text-ink font-mono">{ph.number}</span>
          ...
          {['Email 1', 'Email 2', 'Email 3'].map((label, i) => (
            <div key={i}>
              <p className="text-[10px] font-bold text-ink-muted uppercase tracking-widest mb-1.5">{label}</p>
              {emails[i]
                ? <span className="text-xs text-violet-700 break-all">{emails[i]}</span>
                : <span className="text-xs text-ink-faint">—</span>}
            </div>
          ))}
        </div>
      ...
      {dncScrubbed && (
        <div className="bg-paper-bone border border-violet-200 rounded-xl p-3">
          {/* Header */}
          <div className="flex items-center justify-between mb-2.5">
            <div className="flex items-center gap-1.5">
              <ShieldCheckIcon weight="light" className="w-3.5 h-3.5 text-violet-600 shrink-0" />
              <span className="text-[10px] font-bold text-ink-muted uppercase tracking-widest">DNC Scrub Result</span>
            </div>
            <button
              onClick={downloadClean}
              className="flex items-center gap-1 text-[10px] text-violet-600 hover:text-violet-700 transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98]"
          ...
          {/* Stats */}
          <div className="flex items-center gap-6 mb-3 pb-3 border-b border-line">
            <div>
              <p className="text-lg font-bold text-ink leading-none font-mono">{phones.length}</p>
              <p className="text-[10px] text-ink-muted mt-0.5">Checked</p>
            </div>
            <div>
              <p className="text-lg font-bold text-tagGreen-text leading-none font-mono">{cleanPhones.length}</p>
              <p className="text-[10px] text-ink-muted mt-0.5">Clean (No Flags)</p>
            </div>
            <div>
              <p className="text-lg font-bold text-tagRed-text leading-none font-mono">{flaggedCount}</p>
              <p className="text-[10px] text-ink-muted mt-0.5">Flagged</p>
            </div>
          </div>

          {/* Phone detail table */}
          {phones.length === 0 ? (
            <p className="text-xs text-ink-faint italic">No phones to display</p>
          ) : (
                ...
                <thead>
                  <tr>
                    {['Phone', 'Type', 'National DNC', 'State DNC', 'DMA', 'Litigator'].map(h => (
                      <th key={h} className="text-left pb-2 pr-4 last:pr-0 text-[10px] font-bold text-ink-muted uppercase tracking-widest whitespace-nowrap">
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {phones.map((ph, i) => (
                    <tr key={i} className={ph.dnc ? 'opacity-60' : ''}>
                      <td className="py-1.5 pr-4 font-mono text-ink whitespace-nowrap">{ph.number}</td>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 px-4 text-center">
      <div className="w-14 h-14 rounded-2xl bg-brand-50 border border-brand-200 flex items-center justify-center mb-4">
        <AddressBookIcon weight="light" className="w-7 h-7 text-brand-600" />
      </div>
      <h3 className="font-display text-base font-semibold text-ink mb-2">No skip trace records yet</h3>
